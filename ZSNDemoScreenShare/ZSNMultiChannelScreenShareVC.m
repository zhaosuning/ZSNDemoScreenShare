//
//  ZSNMultiChannelScreenShareVC.m
//  ZSNDemoScreenShare
//
//  Created by zhaosuning on 2023/5/19.
//
...
@interface ZSNMultiChannelScreenShareVC ()<AgoraRtcEngineDelegate>
...
@property (nonatomic, strong) UIView * remoteScreenShareView;

@property (nonatomic, strong) RPSystemBroadcastPickerView * systemBroadcastPicker;

@end

@implementation ZSNMultiChannelScreenShareVC
...
- (void)viewDidLoad {
    ...
    self.remoteScreenShareView = [[UIView alloc] init];
    self.remoteScreenShareView.backgroundColor = [UIColor grayColor];
    //self.remoteView.frame = CGRectMake(0, [[UIScreen mainScreen] bounds].size.width + 100, 200, 200);
    self.remoteScreenShareView.frame = CGRectMake(0, 540, 200, 200);
    [viewbg addSubview:self.remoteScreenShareView];
    ...
    btnStartScreenShare.frame = CGRectMake(0, 190, 170, 40);
    ...
    UIButton *btnShowScreenShare = [UIButton buttonWithType:UIButtonTypeCustom];
    btnShowScreenShare.frame = CGRectMake(0, 240, 200, 40);
    [btnShowScreenShare setTitle:@"显示屏幕共享画面" forState:UIControlStateNormal];
    [btnShowScreenShare setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    btnShowScreenShare.backgroundColor = [UIColor greenColor];
    btnShowScreenShare.titleLabel.font = [UIFont systemFontOfSize:17.0];
    [btnShowScreenShare addTarget:self action:@selector(btnShowScreenShareAction:) forControlEvents:UIControlEventTouchUpInside];
    [viewbg addSubview:btnShowScreenShare];
...
-(void)showRemoteScreenShareVideo:(NSInteger) uid {
    AgoraRtcVideoCanvas *remoteVideo = [[AgoraRtcVideoCanvas alloc] init];
    remoteVideo.uid = uid;
    remoteVideo.view = self.remoteScreenShareView;
    remoteVideo.renderMode = AgoraVideoRenderModeFit;
    [self.agoraKit setupRemoteVideo:remoteVideo];
...
-(void)prepareSystemBroadcaster {
    ...
    //self.systemBroadcastPicker.preferredExtension = [NSString stringWithFormat:@"%@.kkkkk",[[NSBundle mainBundle] bundleIdentifier]];
...
-(void)btnShowScreenShareAction :(UIButton *) button {
    [self showRemoteScreenShareVideo:100];
}
...
- (void)rtcEngine:(AgoraRtcEngineKit *)engine didJoinedOfUid:(NSUInteger)uid elapsed:(NSInteger)elapsed {
    if (uid == 100) {
        [self showRemoteScreenShareVideo:uid];
    }
    else {
...
- (void)rtcEngine:(AgoraRtcEngineKit *)engine localVideoStateChangedOfState:(AgoraVideoLocalState)state error:(AgoraLocalVideoStreamError)error sourceType:(AgoraVideoSourceType)sourceType {
    ...
    if(state == AgoraVideoLocalStateCapturing && sourceType == AgoraVideoSourceTypeScreen) {
        ...
        [self.agoraKit joinChannelExByToken:Token connection:con delegate:self mediaOptions:option joinSuccess:nil];
    }
